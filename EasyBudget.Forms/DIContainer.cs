...
namespace EasyBudget.Forms
{
    public sealed class DIContainer
    {
        ...
        public object Create(Type type)
        {
            ...
            var _constructors = typeInfo.DeclaredConstructors.Where(c => c.IsPublic).ToArray();
            var _constructor = _constructors.FirstOrDefault(c => c.GetParameters().Length == 0);
            if (_constructor != null)
                return Activator.CreateInstance(type);

            // Pick the first constructor found and create any parameters.
            _constructor = _constructors[0];
